        ...
        for obj in self.browse(self._ids):
            ...
            for acc_id in data:
                ...
                for i in self.folio_id.pos_order_ids:
                    invoices = self.env['account.move'].search([('invoice_origin', '=', obj.folio_id.name)])

                    for pos_line in i.lines:
                        print("Product ID:", pos_line.product_id.id)

                        product_invoiced = False
                        for invoice_data in invoices:
                            for invoice_line in invoice_data.invoice_line_ids:
                                if invoice_line.product_id.id == pos_line.product_id.id:
                                    product_invoiced = True
                                    break
                            if product_invoiced:
                                break

                        if product_invoiced:
                            print("Product already invoiced.")
                            i.is_payatcheckout = False
                            i.write({"is_payatcheckout": False})
                        else:
                            print("Product not invoiced.")
                            i.is_payatcheckout = True
                            i.write({"is_payatcheckout": True})
