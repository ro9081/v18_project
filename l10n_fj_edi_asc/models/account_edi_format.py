...
class AccountEdiFormat(models.Model):
    ...
    def _is_compatible_with_journal(self, journal):
        ...
        return journal.country_code == 'US' and journal.type == 'sale'
    ...
    def _get_move_applicability(self, move):
        ...
        # if move.is_sale_document(include_receipts=True) and move.country_code == 'US' and is_under_gst and move.l10n_in_gst_treatment in (
        #     "regular",
        #     "composition",
        #     "overseas",
        ...
        # ):
            ...
            # return {
            #     'post': lambda invoice: self._l10n_in_edi_post_invoice(invoice)
            # }
            return {
    ...
    def _needs_web_services(self):
        self.ensure_one()
        return self.code == "fj_einvoice_1_03" or super()._needs_web_services()
    ...
    def _check_move_configuration(self, move):
        ...
        for line in move.invoice_line_ids.filtered(lambda line: line.display_type not in ('line_note', 'line_section', 'rounding') and not self._l10n_in_is_global_discount(line)):
            ...
            #         # or not any(move_line_tag.id in all_base_tags for move_line_tag in line.tax_tag_ids)):
            #     error_message.append(_(
    ...
    def _l10n_in_edi_post_invoice(self, invoice):
        ...
        if not response.get("error"):
            ...
            invoice_name = invoice.name or invoice.ultiqa_seq_proforma
            json_name = "%s_einvoice.json" % (invoice_name.replace("/", "_"))
    ...
    #                 break
    #     return json_payload
    def load_pfx(self, config):
        try:
            if not config.ultiqa_pfx_path:
                raise UserError("No PFX certificate uploaded in POS config.")
            if not config.ultiqa_pfx_password:
                raise UserError("No PFX password provided in POS config.")
            pfx_data = base64.b64decode(config.ultiqa_pfx_path)
            private_key, cert, _ = load_key_and_certificates(
                pfx_data,
                config.ultiqa_pfx_password.encode(),
                backend=default_backend()
            )
            if not private_key or not cert:
                raise UserError(_("Invalid PFX content. Make sure it includes both private key and certificate."))
            ...
            raise UserError(("Failed to load PFX certificate: %s") % str(e))

    def send_invoice_to_vsdc(self, invoice):
        self.ensure_one()
        pos_order = self.env['pos.order'].search([('account_move', '=', invoice.id)], limit=1)
        config = pos_order.config_id if pos_order else self.env['pos.config'].sudo().search([('name', '=', 'Default')], limit=1)
        ultiqa_pfx_path = config.ultiqa_pfx_path
        ultiqa_pfx_password = config.ultiqa_pfx_password
        ultiqa_pac_value = config.ultiqa_pac_value
        ultiqa_base_url = config.ultiqa_base_url
        if not ultiqa_pfx_path or not ultiqa_pfx_password or not ultiqa_pac_value:
            raise UserError(_("Missing PFX certificate path, password, or PAC value."))
        cert_file, key_file = self.load_pfx(config)
        employee = self.env['hr.employee'].search([('user_id', '=', self.env.uid)], limit=1).ultiqa_cashier
        has_downpayment = any(
            line.product_id and line.product_id.name.lower() == "down payment (pos)"
            for line in invoice.invoice_line_ids
            if line.display_type not in ('line_note', 'line_section', 'rounding'))
        if invoice.state == 'draft':
            invoice_type = "Proforma"
        elif has_downpayment:
        ...
        original_invoice = self.env['account.move'].search(copy_invoice_domain, limit=1)
        if original_invoice:
            if invoice.move_type == "out_refund":
                invoice_type = "Copy"
            else:
                invoice_type = "Copy"
        if invoice_type == "Proforma" and invoice.state == 'draft' and invoice.move_type == "out_refund":
            domain_proforma_refund = [
                ('partner_id', '=', invoice.partner_id.id),
                ('id', '=', invoice.id),
                ('amount_total', '=', invoice.amount_total),
                ('amount_total', '!=', 0.0),
                ('invoice_date', '<=', invoice.invoice_date),
                ('ultiqa_ref_doc_number', '!=', False)]
            original_invoice = self.env['account.move'].search(domain_proforma_refund, limit=1)
        elif invoice_type == "Normal" and invoice.state == 'posted' and invoice.move_type != "out_refund":
            domain_proforma = [
                ('partner_id', '=', invoice.partner_id.id),
                ('id', '=', invoice.id),
                ('amount_total', '=', invoice.amount_total),
                ('ultiqa_ref_doc_number', '!=', False)]
            original_invoice = self.env['account.move'].search(domain_proforma, limit=1)
        elif invoice_type == "Normal" and invoice.state == 'posted' and invoice.move_type == "out_refund":
            domain_refund_sale = [
                ('partner_id', '=', invoice.partner_id.id),
                ('state', '=', 'posted'),
                ...
                ('invoice_date', '<=', invoice.invoice_date),
                ('ultiqa_ref_doc_number', '!=', False),]
            original_invoice = invoice.reversed_entry_id or self.env['account.move'].search(domain_refund_sale, limit=1)
        elif invoice_type == "Advance" and invoice.invoice_line_ids.product_id[0].name == 'Down Payment (POS)':
            domain_advance_sale = [
                ('partner_id', '=', invoice.partner_id.id),
                ('state', '=', 'posted'),
                ('invoice_date', '<=', invoice.invoice_date),
            ...
            original_invoice = invoice.reversed_entry_id or self.env['account.move'].search(domain_advance_sale,limit=1,order="invoice_date desc")
        try:
            transaction_type = "Refund" if invoice.move_type == 'out_refund' else "Sale"
            invoice_data = {
                "dateAndTimeOfIssue": fields.Datetime.now().isoformat(),
                "cashier": employee or '',
                # "buyerId": invoice.partner_id.vat or 'N/A',
                # "buyerCostCenterId": "567546",
                "invoiceType": invoice_type,
                "transactionType": transaction_type,
                "payment": ([{"amount": invoice.amount_total, "paymentType": payment.payment_method_id.vms_payment_id.name or 'Cash'} for payment in pos_order.payment_ids]) or [{"amount": invoice.amount_total, "paymentType": "Cash"}],
                "invoiceNumber": invoice.name if invoice.name else invoice.ultiqa_seq_proforma,
                "options": {"omitQRCodeGen": "0", "omitTextualRepresentation": "0"},
                "items": [],
            }
            if invoice.partner_id.vat and invoice.partner_id.company_type == 'company':
                invoice_data["buyerId"] = invoice.partner_id.vat
                invoice_data["buyerCostCenterId"] = invoice.partner_id.ultiqa_buyer_costcenter
            ...
            if original_invoice:
                invoice_data["referentDocumentNumber"] = original_invoice.ultiqa_ref_doc_number
                invoice_data["referentDocumentDT"] = original_invoice.invoice_date.isoformat()
            headers = {"Content-Type": "application/json", "PAC": ultiqa_pac_value.strip()}
            response = requests.post(
                f"{ultiqa_base_url}/api/v3/invoices",
                headers=headers,
            ...
            if response.status_code == 200:
                if "application/json" in response.headers.get("Content-Type", ""):
                    ...
                    _logger.info(f"Json Response: {resp_data}")
                    invoice_number = resp_data.get("invoiceNumber")
                    if invoice_number and not invoice.ultiqa_ref_doc_number:
                        invoice.ultiqa_ref_doc_number = invoice_number
                    return resp_data
                else:
                    raise UserError(_("200 OK but response is not JSON.\n%s") % response.text[:200])
            elif response.status_code == 403:
                raise UserError(_("403 Forbidden:\n%s") % response.text)
            else:
                raise UserError(
                    _("Invoice submission failed with status %s:\n%s") % (response.status_code, response.text))
        except Exception as e:
            _logger.error("Exception during invoice submission: %s", str(e))
            raise UserError(_("Exception during invoice submission: %s") % str(e))
        finally:
            for f in [cert_file, key_file]:
                try:
                    ...
                    _logger.error("Cleanup failed: %s", cleanup_err)
    ...
    def _l10n_in_edi_generate_invoice_json(self, invoice):
        ...
        json_payload = self.send_invoice_to_vsdc(invoice)
